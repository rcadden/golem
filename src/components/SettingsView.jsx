                    ...
                    return (
                      <li key={m} className="flex items-center justify-between py-2.5 first:pt-0 last:pb-0">
                        <div className="flex items-center gap-2 min-w-0 mr-3">
                          <span className="font-mono text-[13px] text-on-surface truncate">{m}</span>
                          {hwInfo && modelSizes[m] && (() => {
                            const bestVram = hwInfo.gpus.reduce((acc, g) => Math.max(acc, g.vramBytes), 0)
                            const light = modelTrafficLight(modelSizes[m], hwInfo.ram.freeBytes, bestVram)
                            return (
                              <span
                                className="shrink-0 w-2 h-2 rounded-full"
                                style={{ background: light.color }}
                                title={`${light.label} · ${fmtBytes(modelSizes[m])}`}
                              />
                            )
                          })()}
                        </div>
          ...
          <div className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-body-md text-on-surface-variant text-sm">Version 0.7.0</div>
              </div>
            </div>
          </div>
        </section>
